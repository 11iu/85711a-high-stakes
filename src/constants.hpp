// Port 9 may be broken!!
// Port 11 broken

const int LEFT_UPPER_PORT = 6;//3
const int LEFT_BACK_PORT = -7;//-1
const int LEFT_FRONT_PORT = -8;//-10
const int RIGHT_UPPER_PORT = -17;//12
const int RIGHT_BACK_PORT = 18;//-11
const int RIGHT_FRONT_PORT = 16;//-20
const int IMU_PORT = 19;
const int INTAKE_PORT = -12;
const int OPTICAL_PORT = 10;
const int LADY_BROWN = -4;
const int VISION_PORT = 0; // TODO: add this
const char CLAMP_PORT = 'a';
const char RIGHT_DOINKER_PORT = 'b';
const char LEFT_DOINKER_PORT = 'c';
const char POTENTIOMETER_PORT = 'h';
const char L_ULTRA_IN_PORT = 'c';  
const char L_ULTRA_OUT_PORT = 'd'; 
const char B_ULTRA_IN_PORT = 'e';
const char B_ULTRA_OUT_PORT = 'f';
const char L_LED_PORT = 'g'; // Not using
const char R_LED_PORT = 'h'; // Not using
const int L_LED_LENGTH = 64; // 28
const int R_LED_LENGTH = 64; // 28
